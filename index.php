    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($name) || empty($email) || empty($password)) {
            ...
            try {
                ...
                // Register user using new function
                $user_data = [
                    'name' => $name,
                    'email' => $email,
                    'phone' => $phone,
                ...
                // Assign EPIS supervisor if validated
                 if ($assignment_validation['valid'] && $epis_supervisor_id > 0) {
                     ...
                     // Additional success message for auto-assignment
                     if ($assignment_validation['auto_assignment']) {
                         $success .= ' Anda telah otomatis terhubung dengan EPIS supervisor melalui referral EPIC.';
                     }
                 }
                
                $success = 'Registration successful! You can now login to your affiliate account.';
